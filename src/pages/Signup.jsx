...
export default function Signup() {
  ...
  const getPasswordStrength = (pass) => {
    ...
    switch (score) {
      case 1:
        return { score: 1, label: "Weak", color: "bg-red-500/70" };
      case 2:
        return { score: 2, label: "Fair", color: "bg-orange-500/70" };
      case 3:
        return { score: 3, label: "Good", color: "bg-indigo-500/70" };
      case 4:
        return { score: 4, label: "Strong", color: "bg-emerald-500/70" };
      default:
        return { score: 0, label: "Weak", color: "bg-red-500/70" };
    }
  };
  ...
  const onSubmit = (data) => {
    setIsLoading(true);
    console.log("Submit Signup Details:", data);
    ...
    <div className="min-h-screen flex relative overflow-hidden font-sans">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">Your account is ready! Please check your email to verify.</p>
        </div>
      </div>
                    ...
                    {[1, 2, 3, 4].map((index) => (
                      <div
                        ...
                        className={`h-full rounded-full transition-all duration-350 ${
                          strength.score >= index ? strength.color : "bg-white/5"
                        }`}
                      />
                    ))}
                  </div>
            ...
            <div className="flex flex-col gap-1">
              <label className="relative flex items-start cursor-pointer select-none">
                <input
                  ...
                  disabled={isLoading}
                />
                <div className="mt-0.5 h-5 w-5 shrink-0 rounded-md border border-white/10 bg-white/[0.02] peer-checked:bg-indigo-500 peer-checked:border-indigo-500 transition-all duration-200 flex items-center justify-center">
                  <Check size={12} className="text-white scale-0 peer-checked:scale-100 transition-transform duration-200" />
                </div>
                <span className="ml-2.5 text-xs text-slate-400 font-medium peer-checked:text-slate-300 leading-normal">
                  I agree to the{" "}
